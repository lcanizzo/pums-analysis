...
def split_original_dictionary(year):
    ...
    dict_year = ''
    int_year = int(year)
    if int_year <= 2017 and int_year >= 2013:
        dict_year = '2013-2017'
    elif int_year == 2018 or int_year == 2019:
        dict_year = year

    original_file = \
        f'./dictionaries/PUMS_Data_Dictionary_{dict_year}.csv'
    vals_file = get_vals_dict_path(year)
    ...
    if dict_year == '2013-2017':
        ...
        manual_values = pd.read_csv(
            './configs/custom_pre_2016_lanp.csv')
...
def create_name_map_json():
    ...
    col_name_map = pd.read_csv(
        './configs/col_name_map.csv',
        index_col='PUMS_COL_NAME',
        usecols=['PUMS_COL_NAME', 'column']
    ).to_dict()['column']
